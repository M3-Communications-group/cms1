...
function make_menu($pid)
{
    ...
    while ($row = mysqli_fetch_array($MyResult)) {
        ...
        $menu_html[$row["level"]] .= '<div class="menu_lvl' . $row["level"] . $is_active . '"><a href="' . $row["filename"] . '?admin_option=' . $row["id"] . (!empty($_GET["common_sense"]) ? "&common_sense=1" : "") . '">' . $row["name"] . '</a></div>';
    }
}
        ...
        if (mysqli_num_rows($MyResult) == 0) {
            ...
            while ($row = mysqli_fetch_array($MyResult)) {
                ...
                if (!empty($row["content_table"]) && $row["id"] == $admin_option) { // ako tova e izbranoto 
                    ...
                    if ($row["perm_add"] > 0) {
                        $menu_viewadd .= '<div class="menu_lvl_viewadd' . (($action == 'add') ? "_active" : "") . '"><a href="' . $row["filename"] . '?admin_option=' . $admin_option . '&action=add&table=' . $table_categories . '&pid=' . $pid . (!empty($_GET["hide_nav"]) ? "&hide_nav=1" : "") . (!empty($_GET["common_sense"]) ? "&common_sense=1" : "") . '">' . $admin_texts[$lang]["add"] . '</a></div>';
                    }
...
function sitemap($table, $fields_to_show, $pid, $level, $fields, $showorder, $start = 0, $limit = 50, $sub_pid = 0, $fields_to_show_join = '', $fields_to_select = '', $custom_where = '', $group_by = '', $show_sum = '')
{
    ...
    if ($level == 0) {
        ...
        echo '<div style="clear: both;"></div><div style="clear: both;"></div><br><div style="clear: both;"></div><table border="0" cellpadding="0" cellspacing="0" class="viewlist">';
