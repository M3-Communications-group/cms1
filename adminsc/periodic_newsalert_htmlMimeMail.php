...
require_once(__DIR__ . '/inc/htmlMimeMail-2.5.1/htmlMimeMail.php');
...
while ($arr = mysqli_fetch_array($result)) {
	...
	if (mysqli_num_rows($r2) > 0) {
		...
		if (!empty($arr2["photo_small"])) {
			$content_html .= '<img src="' . $arr2["photo_small"] . '" alt="" border="0" style="display: block; float: left; width: 245px; margin: 0px 15px 3px 0px;">';
		}
		$big_photo_html = '';
		$big_photo_txt = '';
		if (!empty($arr2["photo_big"])) {
			$big_photo_html = '<br>For higher resolution photo <a href="' . $arr2["photo_big"] . '" style="color: #000000; text-decoration: underline;">click here</a>';
			$big_photo_txt = '\nHigher resolution photo - ' . $arr2["photo_big"];
		}
